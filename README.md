...
> Третий экран, «Публичные референсы», сохраняет ручные наблюдения без парсинга и переносит в brief только свой паттерн с reference-only boundary.
> Четвёртый экран, «Планировщик контента», создаёт локальные draft/review-задачи с evidence, KPI, effort и обязательным human approval — без scheduler и publish API.

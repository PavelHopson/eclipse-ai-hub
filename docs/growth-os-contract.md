...
## Hook Vault — первый экран Content Command Center

`growth.hook.v1` хранит не чужой контент, а короткий reusable pattern с обязательными полями:
HTTPS source URL без credentials, author, captured date, rights status и редакторская заметка.
Хранилище ограничено 30 записями и 64 КБ и существует только в `localStorage` текущего браузера.

Кнопка «В brief» переносит provenance и ограничения в форму Growth OS. Она не открывает ссылку,
не запускает AI-запрос, не копирует исходный текст или визуал и не разрешает публикацию. Записи
с неизвестными правами остаются reference до ручной проверки.

## Channel Analytics — второй экран Content Command Center

`growth.channel-snapshot.v1` хранит агрегированный результат одного продукта и канала за окно
7, 30 или 90 дней. Обязательны дата конца периода, число публикаций, показы, медиана, сохранения,
клики, переходы в продукт, целевые обращения, HTTPS evidence и заметка о методе подсчёта.

Данные остаются в `localStorage` текущего браузера: максимум 24 периода и 64 КБ. Парсер считает
хранилище недоверенным, отклоняет лишние поля, небезопасные символы, credentials в URL, будущие
даты, дубликаты и невозможную воронку. Динамика считается только относительно предыдущего периода
с теми же product, channel и window. Разные каналы не складываются в общую метрику.
...
Таблица, Канбан и Календарь — синхронные проекции одного массива записей. Поиск и фильтры не
копируют данные и не меняют authority; на узких экранах таблица и календарь заменяются карточками
без горизонтального scroll. Сводки считаются локально и не являются внешней аналитикой.

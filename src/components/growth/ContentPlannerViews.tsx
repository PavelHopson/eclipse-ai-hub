...
function StatusBadge({ item }: { item: ContentPlannerItem }) {
  ...
  return <span className={`inline-flex rounded-full border px-2 py-1 text-[10px] font-semibold ${ready ? 'border-emerald-300/30 bg-[#0c4935] text-[#c4ffe2]' : 'border-white/10 bg-[#182231] text-gray-200'}`}>{ready ? 'К review' : 'Черновик'}</span>;
}
...
function PlannerCard(props: PlannerViewProps & { item: ContentPlannerItem; compact?: boolean }) {
  ...
  return (
    <article className="planner-card-motion group rounded-xl border border-hub-border bg-black/10 p-4 transition-[border-color,background-color,box-shadow,transform] hover:-translate-y-px hover:border-hub-accent/35 hover:bg-hub-accent/[0.04] hover:shadow-[0_14px_32px_-24px_rgba(107,163,255,.55)] focus-within:border-hub-accent/45">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-hub-accent">{item.product}</div><h4 className="mt-1 font-medium text-white">{item.workingTitle}</h4><p className="mt-1 text-xs text-gray-400">{CHANNEL_LABEL[item.channel]} · {FORMAT_LABEL[item.format]} · effort {item.effort}</p></div>
        <StatusBadge item={item} />
      </div>
      <div className={`mt-4 grid gap-3 text-xs leading-5 ${compact ? '' : 'sm:grid-cols-2'}`}>
        <div><div className="font-medium text-gray-300">Цель</div><div className="text-gray-400">{item.goal}</div></div>
        ...
        {!compact && <><div><div className="font-medium text-gray-300">Аудитория</div><div className="text-gray-400">{item.audience}</div></div><div><div className="font-medium text-gray-300">CTA</div><div className="text-gray-400">{item.cta}</div></div></>}
      </div>
...
export function ContentPlannerTable(props: PlannerViewProps) {
  return (
    <>
      <div className="hidden overflow-hidden rounded-xl border border-hub-border bg-black/[0.08] xl:block">
        <table className="w-full table-fixed border-separate border-spacing-0 text-left text-xs">
          ...
          <thead className="bg-[#111820]"><tr className="text-[10px] uppercase tracking-[0.14em] text-gray-300"><th className="border-b border-hub-border px-3 py-3">Материал</th><th className="border-b border-hub-border px-3 py-3">Канал</th><th className="border-b border-hub-border px-3 py-3">Review</th><th className="border-b border-hub-border px-3 py-3">Статус</th><th className="border-b border-hub-border px-3 py-3">Действия</th></tr></thead>
          <tbody>{props.items.map((item) => { const overdue = item.reviewOn < props.today; return <tr key={item.id} className="planner-row-motion group align-top transition-colors odd:bg-white/[0.012] hover:bg-hub-accent/[0.055] focus-within:bg-hub-accent/[0.045]"><td className="border-b border-hub-border/70 px-3 py-4"><div className="font-medium text-white">{item.workingTitle}</div><div className="mt-1 truncate text-gray-400">{item.product} · {item.goal}</div></td><td className="border-b border-hub-border/70 px-3 py-4 text-gray-400">{CHANNEL_LABEL[item.channel]}<div className="mt-1 text-gray-500">{FORMAT_LABEL[item.format]} · {item.effort}</div></td><td className={`border-b border-hub-border/70 px-3 py-4 ${overdue ? 'font-medium text-amber-300' : 'text-gray-400'}`}>{formatDate(item.reviewOn)}{overdue && <div className="mt-1 text-[10px]">Просрочено</div>}</td><td className="border-b border-hub-border/70 px-3 py-4"><StatusBadge item={item} /></td><td className="border-b border-hub-border/70 px-3 py-3"><PlannerActions {...props} item={item} /></td></tr>; })}</tbody>
        </table>
      </div>
...
export function ContentPlannerBoard(props: PlannerViewProps) {
  ...
  return <div className="grid gap-4 lg:grid-cols-2">{columns.map((column) => { const items = props.items.filter((item) => item.status === column.status); return <section key={column.status} className="planner-column-motion rounded-xl border border-hub-border bg-black/[0.08] p-3"><header className="mb-3 flex items-center justify-between gap-3 px-1"><div><h4 className="text-sm font-semibold text-white">{column.title}</h4><p className="mt-1 text-[11px] text-gray-400">{column.hint}</p></div><span className="rounded-full border border-hub-border px-2 py-1 text-[10px] text-gray-400">{items.length}</span></header><div className="space-y-3">{items.length ? items.map((item) => <PlannerCard key={item.id} {...props} item={item} compact />) : <div className="rounded-lg border border-dashed border-hub-border p-5 text-center text-xs text-gray-500">В этой колонке пока нет задач.</div>}</div></section>; })}</div>;
}
  ...
  return (
    <div>
      ...
      <div className="hidden grid-cols-7 gap-px overflow-hidden rounded-xl border border-hub-border bg-hub-border lg:grid">
        {['Пн', 'Вт', 'Ср', 'Чт', 'Пт', 'Сб', 'Вс'].map((day) => <div key={day} className="bg-hub-surface px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-gray-400">{day}</div>)}
        {days.map((day) => { const tasks = byDate.get(day.isoDate) ?? []; return <div key={day.isoDate} className={`planner-calendar-day min-h-28 bg-hub-card p-2 ${day.inCurrentMonth ? '' : 'opacity-55'} ${day.isoDate === props.today ? 'ring-1 ring-inset ring-hub-accent/60' : ''}`}><div className="text-[10px] text-gray-400">{formatDate(day.isoDate)}</div><div className="mt-2 space-y-1">{tasks.slice(0, 3).map((item) => <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`planner-calendar-item block w-full truncate rounded border px-2 py-1.5 text-left text-[10px] ${item.status === 'ready-for-review' ? 'border-emerald-300/25 bg-[#0c4935] text-[#c4ffe2]' : 'border-hub-border bg-[#111820] text-gray-200'} hover:border-hub-accent/50 hover:bg-hub-accent/[0.07]`}>{item.workingTitle}</button>)}{tasks.length > 3 && <div className="px-1 text-[10px] text-gray-500">+ ещё {tasks.length - 3}</div>}</div></div>; })}
      </div>
      <div className="grid gap-3 lg:hidden">{monthItems.length ? monthItems.map((item) => <PlannerCard key={item.id} {...props} item={item} />) : <div className="rounded-xl border border-dashed border-hub-border p-5 text-center text-xs text-gray-500">В этом месяце задач нет.</div>}</div>
      {selected && <div className="planner-view-motion mt-4 hidden lg:block"><PlannerCard {...props} item={selected} /></div>}
    </div>
  );
}

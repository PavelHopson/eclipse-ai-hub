...
export function ContentPlannerWorkspace(props: ContentPlannerWorkspaceProps) {
  ...
  const summaryCards: Array<{ id: PlannerStatusFilter; label: string; value: number; hint: string; tone: string }> = [
    ...
    { id: 'overdue', label: 'Просрочено', value: summary.overdue, hint: 'Не скрываются', tone: summary.overdue ? 'text-amber-300' : 'text-gray-400' },
  ];
  ...
  return (
    <section className="mt-6 border-t border-hub-border pt-5" aria-labelledby="planner-queue-title">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div><h3 id="planner-queue-title" className="text-sm font-semibold text-white">Редакторская очередь</h3><p className="mt-1 text-xs leading-5 text-gray-400">Выберите удобный вид. Review остаётся запросом на проверку, а не разрешением публиковать.</p></div>
        <div className="inline-flex w-fit rounded-lg border border-hub-border bg-black/10 p-1" aria-label="Представление задач">{VIEW_OPTIONS.map(({ id, label, icon: Icon }) => <button key={id} type="button" onClick={() => setView(id)} aria-pressed={view === id} className={`inline-flex items-center gap-2 rounded-md px-3 py-2 text-xs font-medium transition-colors ${view === id ? 'bg-hub-accent/10 text-hub-accent shadow-[inset_0_0_0_1px_rgba(107,163,255,.2)]' : 'text-gray-400 hover:text-gray-200'}`}><Icon size={14} /><span className="hidden sm:inline">{label}</span></button>)}</div>
      </div>

      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{summaryCards.map((card) => <button key={card.id} type="button" onClick={() => setStatusFilter(card.id)} aria-pressed={statusFilter === card.id} className={`planner-summary-card rounded-xl border p-4 text-left ${statusFilter === card.id ? 'border-hub-accent/35 bg-hub-accent/[0.045]' : 'border-hub-border bg-black/10 hover:border-hub-accent/20'}`}><div className="flex items-baseline justify-between gap-3"><span className="text-xs font-medium text-gray-400">{card.label}</span><strong className={`text-xl ${card.tone}`}>{card.value}</strong></div><div className="mt-2 text-[11px] text-gray-500">{card.hint}</div></button>)}</div>

      <div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_190px]">
        <label className="relative block"><span className="sr-only">Найти задачу</span><Search size={15} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="hub-input !py-2.5 !pl-9" placeholder="Найти материал, продукт или CTA…" /></label>
      ...
      <div className="mt-3 flex items-center justify-between gap-3 text-[11px] text-gray-500"><span>Показано {visibleItems.length} из {props.items.length}</span>{(query || channelFilter !== 'all' || statusFilter !== 'all') && <button type="button" className="text-hub-accent hover:text-white" onClick={() => { setQuery(''); setChannelFilter('all'); setStatusFilter('all'); }}>Сбросить фильтры</button>}</div>

      <div key={view} className="planner-view-motion mt-4">
        {props.items.length === 0 ? <div className="rounded-xl border border-dashed border-hub-border p-7 text-center"><CalendarRange size={22} className="mx-auto text-hub-accent" /><div className="mt-3 text-sm font-medium text-gray-300">Очередь пока пуста</div><p className="mx-auto mt-2 max-w-lg text-xs leading-5 text-gray-500">Добавьте первую задачу: один материал, одно evidence, один CTA и дата следующего решения.</p></div>
          : visibleItems.length === 0 ? <div className="rounded-xl border border-dashed border-hub-border p-7 text-center"><Search size={22} className="mx-auto text-gray-500" /><div className="mt-3 text-sm font-medium text-gray-300">По этим условиям ничего не найдено</div><button type="button" className="mt-3 text-xs text-hub-accent hover:text-white" onClick={() => { setQuery(''); setChannelFilter('all'); setStatusFilter('all'); }}>Показать все задачи</button></div>
